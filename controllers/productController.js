...
export const realtedProductController = async (req, res) => {
  try {
    console.log("related Product function triggerd");
    const { pid, cid } = req.params;
    console.log(cid + "-----------this is catefory id");
    const products = await productModel
      .find({
        category: cid,
        _id: { $ne: pid },
    ...
    res.status(200).send({
      success: true,
      message: "Similar product fetched sucsessfully",
      products,
    });
...
export const singleCategoryProductsController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      message: "Error while getting products of single category",
      error,
    });
  }
};
...
export const braintreeTokenController = async (req, res) => {
  try {
    console.log("Payment gateway api controller function triggered");
